        ...
        if not hasattr(self, '_cached_latest_version'):
            if hasattr(self, 'prefetched_versions'):
                ...
                self._cached_latest_version = self.versions.filter(
                    start_date__lte=date.today()).first()
...
class HandbookVersion(models.Model):
    ...
    handbook = models.ForeignKey(Handbook, on_delete=models.CASCADE,
                                 ...
                                 verbose_name=_('Handbook'))
    version = models.CharField(verbose_name=_('Handbook Version'),
                               max_length=50)
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=['handbook', 'start_date'],
                                    name='unique_version_start_date')
        ]
...
class HandbookElement(models.Model):
    ...
    """
    version = models.ForeignKey(HandbookVersion, on_delete=models.CASCADE,
